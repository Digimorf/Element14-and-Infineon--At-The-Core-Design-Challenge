...
/*************************************************************/
void *WrZ80_Fn_Ptr;
void (*WrZ80_Fn) (register word, register byte);
void *RdZ80_Fn_Ptr;
uint8_t(*RdZ80_Fn) (register word);
void *OutZ80_Fn_Ptr;
void (*OutZ80_Fn) (register word, register byte);
void *InZ80_Fn_Ptr;
uint8_t(*InZ80_Fn) (register word);
    ...
    default:
        if (R->TrapBadOps)
            ...
            ("[Z80 %lX] Unrecognized instruction: DD CB %02X %02X at PC=%04X\n", (long)(R->User),
             OpZ80(R->PC.W - 2), OpZ80(R->PC.W - 1), R->PC.W - 4);
    }
#undef XX
}
...
#include "CodesED.h"
    ...
    default:
        if (R->TrapBadOps)
            printf("[Z80 %lX] Unrecognized instruction: ED %02X at PC=%04X\n", (long)R->User,
                   OpZ80(R->PC.W - 1), R->PC.W - 2);
    }
}
...
void ResetZ80(Z80 * R)
{
    ...
    WrZ80_Fn = WrZ80_Fn_Ptr;
    RdZ80_Fn = RdZ80_Fn_Ptr;
    OutZ80_Fn = OutZ80_Fn_Ptr;
    InZ80_Fn = InZ80_Fn_Ptr;
}
...
/*************************************************************/
inline void IntZ80(Z80 * R, word Vector)
{
    ...
    if ((R->IFF & IFF_1) || (Vector == INT_NMI))
    {
        ...
        if (R->IAutoReset && (Vector == R->IRequest))
            R->IRequest = INT_NONE;
...
#include <Codes.h>
        ...
        if (R->ICount <= 0)
        {
            ...
            if (R->IFF & IFF_EI)
            {
                R->IFF = (R->IFF & ~IFF_EI) | IFF_1;	/* Done with AfterEI
														   state */
